...
impl <'cmd> DebugConsole<'cmd> {
    ...
    pub fn parse_cmd<'a>(line: &'a [u8], token_buffer: &'a mut [CommandToken]) -> &'a[CommandToken]
    {
    ...
    pub fn execute_cmd(&self, line: &[u8]) -> Result<(), ErrorCode> {
        if line.len() > 0 {
            ...
            /*
            for token in args {
                if let Some(text) = token.get_str(&line) {
                    write!(Console::writer(), "pos:{} len:{} text:[{}]\r\n", token.position, token.length, text).unwrap();
                }
            }
            */
            // execute command
            if let Some(command_name) = args[0].get_str(&line) {
                ...
                write!(Console::writer(), "\nUnknown command!!!:{}\r", command_name).unwrap();
            }
        }
        Ok(())
    }
